...
class Facebook extends Response {
    ...
    public function __construct() {
        ...
        $this->clientId = &$params['service:facebook:client_id'];
        $this->sharedSecret = &$params['service:facebook:secure_key'];
    }
    ...
    public function getRoutes() {
        ...
        $group->addGet('/', function($req, $next) {
            ...
            $routes = [
                'tokens' => Router::byName('API>Auth>V1>Services>Facebook>Tokens')
                    ->getAbsolutePath()
            ];
        ...
        })->setName('API>Auth>V1>Services>Facebook');

        $group->addGet('/tokens', [$this, 'getTokens'])
            ->setName('API>Auth>V1>Services>Facebook>Tokens');
    ...
    function getTokens($req, $next) {
        ...
        try {
            ...
            $facebookUserId = $this->getUserId();

            if ($facebookUserId !== null) { 

                $user = \Model\User::find()
                    ->andWhere('JSON_EXTRACT(ids, \'$.facebook\') = :facebookUserId', [
                ...
                if ($user->isNew()) {
                    ...
                    $serviceIds->facebook = $facebookUserId;
